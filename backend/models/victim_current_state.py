...
class VictimCurrentState(Base):
    ...
    altitude_m = Column(Float, nullable=True, comment='latest altitude in metres above ground')
    rssi = Column(Float, nullable=True, comment='latest signal strength in dBm')
    uav_relay_id = Column(Text, nullable=True, comment='primary UAV relay serving this victim')
    uav_backup_ids = Column(Text, nullable=True, default='[]', comment='JSON array of backup UAV IDs')
    current_region = Column(Text, nullable=True, comment='operational region for this user')
    emergency_status = Column(Text, nullable=True, default='normal', comment='normal | victim | rescued | deceased')
