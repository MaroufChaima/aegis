...
export default function SensorStatusPanel({ assignedSensors = [], victim }) {
  ...
  if (wearables.length === 0 && !victim) {
    return <div className="text-xs text-slate-500 dark:text-gray-400">No wearable data available</div>
  }

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-800 dark:text-gray-200">Wearable Sensors</h3>
        {wearables.length > 0 && (
      ...
      <div className="grid grid-cols-2 gap-2 p-2 bg-slate-50 dark:bg-gray-700/30 rounded border border-slate-200 dark:border-gray-600/50">
        <div>
          <p className="text-xs text-slate-500 dark:text-gray-500">Coordinator link RSSI</p>
          <p className="text-sm font-semibold text-slate-900 dark:text-white">
            {victim?.rssi != null ? `${victim.rssi.toFixed(1)} dBm` : '—'}
          </p>
        </div>
        <div>
          <p className="text-xs text-slate-500 dark:text-gray-500">UAV relay</p>
          <p className="text-sm font-semibold text-slate-900 dark:text-white font-mono">
            {victim?.uav_relay_id || '—'}
          </p>
        </div>
        <div>
          <p className="text-xs text-slate-500 dark:text-gray-500">Packet quality</p>
        ...
        <div>
          <p className="text-xs text-slate-500 dark:text-gray-500">Coordinator battery</p>
          <p className={`text-sm font-semibold ${
            victim?.battery != null && victim.battery < 20
      ...
      {wearables.length > 0 ? (
        <div className="rounded border border-slate-200 dark:border-gray-600 overflow-hidden">
          <div className="grid grid-cols-3 gap-2 px-2 py-1.5 bg-slate-100 dark:bg-gray-700/60 text-xs text-slate-500 dark:text-gray-400 font-medium">
            <span>Sensor</span>
            <span>Status</span>
          ...
          {wearables.map(({ id, label }) => {
            ...
            return (
              <div
                key={id}
                className="grid grid-cols-3 gap-2 px-2 py-2 border-t border-slate-200 dark:border-gray-700/60 items-center"
              >
                <span className="text-xs text-slate-800 dark:text-gray-200">{label}</span>
                <div className="flex items-center gap-1.5">
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 ${getStatusDotColor(status)}`} />
                  <span className="text-xs text-slate-500 dark:text-gray-400">{getStatusLabel(status)}</span>
                </div>
        ...
        <p className="text-xs text-slate-500 dark:text-gray-500">No sensors assigned to this victim.</p>
      )}
    </div>
  )
}
