...
/**
 * Shows a rolling history of vital changes for the selected victim.
 */
export default function VictimHistoryModal({ victim, history = [], onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-gray-800 border border-gray-600 rounded-lg shadow-xl w-full max-w-2xl max-h-[85vh] overflow-y-auto p-4"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-sm font-semibold text-white">Vital History</h2>
        ...
        {history.length === 0 ? (
          ...
          <div className="rounded border border-gray-600 overflow-x-auto">
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-gray-700/60">
                {history.map((row, i) => (
                  <tr key={`${row.timestamp}-${i}`} className="text-gray-200">
